...
| Version | Summary |
|---|---|
| v1.87 | **HUD polish and a visual pass.** An **active-status chip row** (HIDDEN, POISON, HUNGRY, WOUNDED, plus **forcefield charge**, **shield wear / mirror heat**, lotus **daze**, **burden** — colour-coded, some gauged) now sits **beside the score** to balance the HUD, with thin **dividers** between HANDS \| POCKETS \| PACK \| WALKMAN. A slim **shield bar** floats over the player's head in the shield's colour, so a **pocketed** shield's condition shows, not only a held one. The little **rubble piles** are now textured like the rocks. And the **title/gate background video** is back — a prior cleanup removed the element believing its clip was uncommitted, but it had been tracked since v1.39. |
